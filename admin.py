...
def require_sales_manager_auth(*, session_timeout_seconds: int = 2 * 60 * 60) -> None:
    ...
    # If already authed, enforce timeout + show logout
...
def render_sales_manager_dashboard(df_sold: pd.DataFrame) -> None:
    ...
    fmt_cols = {
        c: "${:,.0f}"
        for c in county_table.columns
        if c in ["Total GP", "Avg GP", "Total Wholesale", "Avg Wholesale"]
    }
